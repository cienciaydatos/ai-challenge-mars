...
class Clusterer:
	def __init__(self, dataset, cnn_architecture, layer, clustering_algorithm, n_classes = 7):
		self.dataset_path = "./%s/" % dataset
		self.dataset_feat_path = self.dataset_path + "Features/%s_%s/" % (cnn_architecture, layer)
		print("dataset_path: " + str(self.dataset_path))
		print("dataset_feat_path: " + str(self.dataset_feat_path))        
        
        self.n_files = len(glob(self.dataset_feat_path + "*.p"))
        print("length", len(self.n_files))
	...
	def get_features(self):
		...
		print("self feature size", self.features.shape)
